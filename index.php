...
if (isset($_GET['search']) && !empty($_GET['keyword'])) {
    $keyword = mysqli_real_escape_string($conn, $_GET['keyword']); // Ngăn chặn SQL Injection
    ...
    if (!$query_timkiem) {
        die("Lỗi truy vấn tìm kiếm: " . mysqli_error($conn));
    }
} else {
    // Nếu không có từ khóa tìm kiếm, hiển thị tất cả sản phẩm
    $sql_timkiem = "SELECT * FROM tbl_product";
    $query_timkiem = mysqli_query($conn, $sql_timkiem);

    // Kiểm tra lỗi trong truy vấn
    if (!$query_timkiem) {
        die("Lỗi truy vấn lấy tất cả sản phẩm: " . mysqli_error($conn));
    }
}
...
<body>
    <div class="banner">
        <div class="navbar">
            <div class="logo">
                <img src="./frontend/img/smalllogo.png" class="logo" style="border-radius: 50%;">
            </div>
            ...
            <li class="search-bar" style="display: inline-block; margin-left: auto; margin-right: 200px;">
                <div class="input-group rounded">
                    <form action="" method="GET" style="display: flex;">
                        <input type="text" name="keyword" value="" style="padding-left: 5px;" placeholder="Tìm kiếm sản phẩm...">
    ...
    <nav class="navbrand navbar-light">
        <div class="container-fluid">
            <?php while ($result = $kq1->fetch_assoc()) { ?>
        ...
        if (mysqli_num_rows($query_timkiem) > 0) {
            while ($rowtimkiem = mysqli_fetch_assoc($query_timkiem)) { ?>
                <div class="container-fluid-bottom-item">
                    <form action="<?php echo $rowtimkiem['product_id']; ?>" method="POST">
                        <img src="./backend/admin/upload_images/<?php echo $rowtimkiem['product_img']; ?>" alt="">
                        <div>
                            <h6><?php echo $rowtimkiem['product_name']; ?></h6>
